---
title: "Part V: The Bayesian analysis of psychometric topology"
...
- In Part I, we explored why network models became a tool in psychology.  
- In Part II, we saw how Bayesian reasoning distinguishes *evidence of absence* from *absence of evidence*.  
- In Part III, we practiced the Bayesian workflow: testing edges, estimating parameters, visualizing uncertainty, and running sensitivity checks.  

# Recap: Part IV — Priors as Models of Psychometric Topology

In Part IV, we explored how a prior can encode ideas about network structures.  

::: {.callout-note title="Recap Questions"}

> If we don’t want to pick π by hand; which prior should we use?

> When comparing Bernoulli vs Beta–Bernoulli, what role does **network complexity** (number of edges) play?  

> The Stochastic Block Model (SBM) adds something new: what kinds of structures can it represent that the other priors cannot?  

:::


# Intended Outcome of Part V

At the end of this session, you should be able to:

::: {.goals}
- **Identify** the structural assumptions built into different edge priors:  
  – Bernoulli: independent edges with fixed probability π.  
  – Beta–Bernoulli: random π with an induced distribution on network complexity.  
  – Stochastic Block Model: explicit modeling of domains and clusters.

- **Explain** how the Beta–Bernoulli prior improves on the Bernoulli prior by:  
  – treating π as random rather than fixed,  
  – balancing the prior weight across different network complexities,  
  – preventing medium-density networks from dominating simply because they are most numerous, and  
  – avoiding arbitrary specification of π.

- **Estimate** psychological networks under different priors (Bernoulli, Beta–Bernoulli, SBM) and compare how prior assumptions affect posterior edge inclusion, network density, and clustering.

- **Interpret** posterior results in light of prior assumptions, with particular attention to:  
  – how the Beta–Bernoulli prior redistributes weight over sparse, medium, and dense networks,  
  – how the SBM prior can reveal clusters and bridge symptoms, and  
  – when a prior favors sparse versus dense structures.
:::
...
### 1.1.1 Simulating the Bernoulli Prior

Under Bernoulli(π), each edge is included independently with fixed probability π.  
This exercise helps us **identify the structural assumptions** of this prior.

**Tasks**

1. **Simulate networks:** Generate several random networks from Bernoulli(π = 0.5) with *p = 10* nodes.  
   *Goal: observe what “independent edges with fixed probability” looks like in practice.*

2. **Check expectations:**  
   - Compute the number of possible edges $m = \tfrac{p(p-1)}{2}$.  
   - Compute the expected number of edges $E[\#\text{edges}] = m \times \pi$.  
   - Compute the expected density $E[\text{density}] = \pi$.  
   *Goal: connect simulation results to theoretical expectations.*

3. **Interpret results:** Compare simulated networks to these expectations.  
   *Goal: see whether the prior behaves as the mathematics predicts.*

4. **Reflect:**  
   - Do the simulated edges appear independent?  
   - Does the expected density change when the number of nodes $p$ increases?  
   *Goal: extract the key assumptions of the Bernoulli prior to contrast later with Beta–Bernoulli.*
...
### 1.1.2 Simulating the Beta–Bernoulli Prior

Under a **Beta–Bernoulli(α,β)** prior:  
- π is not fixed but drawn from a Beta distribution.  
- Each edge is then included independently with probability π.  

This exercise helps us **explain how the Beta–Bernoulli prior generalizes the Bernoulli prior** by allowing networks of varying densities.

**Tasks**

1. **Simulate networks:** Generate several random networks with *p = 10* under Beta–Bernoulli(1,1).  
   *Goal: see how making π random produces both sparse and dense networks, not just ones near 50% density.*

2. **Check expectations:**  
   - Compute the number of possible edges $m = \tfrac{p(p-1)}{2}$.  
   - Compute expected edge count: $E[\#\text{edges}] = E[π] \times m$.  
   - Compute expected density: $E[\text{density}] = E[π]$.  
   *Goal: connect simulations to the role of the Beta distribution for π.*

3. **Interpret results:** Compare simulated networks to these expectations.  
   *Goal: evaluate how much variety in density arises compared to Bernoulli(0.5).*

4. **Explore α, β:** Try different settings such as (2,5), (5,2), (10,10).  
   - How does the distribution of π change?  
   - How do simulated densities change?  
   - Which choices favor sparser vs. denser networks?  
   *Goal: understand how α and β encode beliefs about sparsity or density.*
...
In practice, the choice of prior is not just a technical detail.  
It reflects your substantive expectations about the network: Is it relatively **sparse** or **dense**?
...
## 1.4. Multiplicity correction in network priors
...
## 1.5. Bonus: Multiple testing in Bayesian statistics
...
# 2. The Stochastic-Block Model and Network Clustering
...
- Understand how the SBM prior encodes clustering.  
- Learn the Bayesian workflow for estimating networks under the SBM prior.  
- Apply the SBM to real data and interpret the clustering output.  
- Reflect on how to set and justify hyperparameters such as *λ* (expected number of clusters).
...
Your task is to build intuition for what the Stochastic Block Model (SBM) prior implies.  

1. **Simulate networks:** Generate networks with three blocks of equal size.  
   - First, try *small blocks* (e.g., `block_sizes = c(4,4,4)`).  
   - Then, try *larger blocks* (e.g., `block_sizes = c(10,10,10)`).  
   *Goal: compare how cluster size affects how clearly clusters are visible.*  

2. **Set probabilities:** Use a high probability of edges *within* blocks (e.g., 0.6) and a low probability *between* blocks (e.g., 0.1).  
   *Goal: see how different within- vs. between-block probabilities create clustered structure.*  

3. **Visualize:** Plot several simulated networks.  
   *Goal: recognize visual evidence of clusters compared to earlier priors.*  

4. **Compare:** Relate your results to the Bernoulli/Beta–Bernoulli networks you generated earlier.  
   *Goal: contrast clustering (SBM) with density-only priors (Bernoulli/Beta–Bernoulli).*  
...
1. **Set block sizes:**  
   ...
   *Goal: compare how cluster size influences how clearly clusters can be detected.*  

2. **Set edge probabilities:**  
   - Use `p_within = 0.6` and `p_between = 0.1` as default values.  
   - Experiment with other values if you like.  
   *Goal: see how differences between within- and between-block probabilities create more or less visible clustering.*  

3. **Simulate multiple networks:**  
   - Use the loop to generate and plot several draws.  
   *Goal: observe variability across draws while the same structural assumptions hold.*  

4. **Compare with earlier priors:**  
   - Recall what Bernoulli and Beta–Bernoulli networks looked like.  
   *Goal: identify that SBM encodes clustering structure, while Bernoulli/Beta–Bernoulli only encode density.*  
...
- **Clusters vs. randomness:**  
  Did the SBM simulations show clusters more clearly than Bernoulli/Beta–Bernoulli?  
  *Goal: confirm that SBM encodes structural organization, not just density.*  

- **Effect of block size:**  
  How did small blocks (4 nodes) compare to larger blocks (10 nodes) in terms of visible clustering?  
  *Goal: understand that detecting clusters requires larger *p* (nodes), not larger *n* (observations).*  

- **Within- vs. between-block probabilities:**  
  How did changing `p_within` and `p_between` affect the clarity of clusters?  
  *Goal: link these parameters to psychological expectations — for example, strong within-block connectivity corresponds to tightly connected symptom domains, while higher between-block connectivity reflects weaker separation between domains.*

- **Connection to psychology:**  
  If these were symptom networks, how would you interpret within-cluster edges vs. between-cluster edges?  
  *Goal: link prior assumptions to substantive expectations about symptom domains and cross-domain connections.*   
...
- Larger blocks make clusters more clearly detectable, highlighting that cluster detection depends on the number of nodes (*p*), not the sample size (*n*).  
...
The simulations showed how SBM priors generate clustered networks.  
We now connect this intuition to the **Bayesian model** used for estimation.  

### The generative model

1. **Block assignments**  
   - Each node is assigned to one of several latent blocks (clusters).  
   - The model places a probability distribution over possible assignments, so that each node is assigned to block $b$ with some probability.  
   - This means that the block structure is not fixed in advance, but learned from the data during inference.

2. **Edge probabilities**  
   - For each pair of blocks $(a,b)$, the probability of an edge between nodes in those blocks is drawn from a Beta distribution.  
   - Conditional on this probability, each edge is then sampled as Bernoulli.  
   - In other words, **every within-block or between-block set of relations follows a Beta–Bernoulli model**, just like the ones we studied earlier.  
   - The difference is that instead of one global $\pi$ for all edges, the SBM places a separate Beta–Bernoulli prior on each block-to-block connection pattern.

3. **Edges**  
   - Once blocks have been assigned and block-to-block probabilities $\pi_{ab}$ have been drawn, each possible edge is sampled as Bernoulli($\pi_{ab}$).  
   - This is exactly the same Bernoulli step we saw earlier, but now the success probability depends on the blocks of the two nodes.  
   - Put differently: **an SBM network is built from many local Beta–Bernoulli priors**, one for each pair of blocks, which together generate the entire network.

This hierarchy means that:  
- Networks inherit **clustering structure** from the block assignments.  
- Both the number of clusters and the block-to-block edge probabilities are treated as random variables; they are not fixed in advance but are estimated from the data.


### Hyperparameters

The SBM prior extends the ideas from the Bernoulli and Beta–Bernoulli priors by introducing parameters that govern clustering as well as density:
...
Unlike the Bernoulli and Beta–Bernoulli priors, which govern overall edge density,  
the SBM prior makes assumptions about **organization**: which nodes are likely to connect, based on latent clusters.
...
*Goal:* see which numbers of clusters (e.g., 1, 2, 3) are most supported by the data.  
...
\[
P(B = b) = \frac{e^{-\lambda}\, \lambda^b / b!}{1 - e^{-\lambda}}, \quad b = 1, 2, \ldots
\]
...
We now turn to an empirical example from Martarelli, Ballifard, and Audrin (2023).  
...
Your task is to test the clustering hypotheses from \textcite{martarelli2022} using the Bayesian SBM.

1. **Specify prior expectations:**  
   - Choose $\lambda$ to reflect the expected number of clusters (the authors expected 3).  
   - Use default values for $\alpha,\beta = 1$ (neutral beliefs about density).  
   - Leave the Dirichlet parameter at 1 (neutral cluster size balance).  
   *Goal: translate a substantive hypothesis into prior parameters.*  

2. **Fit the SBM model:**  
   Estimate the model on the cleaned dataset using `bgm()`.  
   *Goal: apply the SBM to empirical data.*  

3. **Compute posterior probabilities:**  
   Extract `posterior_num_blocks` to get the posterior distribution over cluster numbers.  
   *Goal: see whether the data support 1, 2, or 3 clusters.*  
...
4. **Compute clustering Bayes factors:**  
   ...
   - $H_1$: two clusters  
   - $H_2$: three clusters  
   *Goal: quantify evidence for 2 vs 3 clusters (the authors’ expectation).*  
...
# 👉 Tasks:
# - Extract p1, p2, p3 (posterior probabilities for 1, 2, 3 clusters).
# - Compute prior1, prior2, prior3 from truncated Poisson(λ = 2.821439).
# - Calculate Bayes factors:
#     BF(2 vs 1), BF(3 vs 2), BF(≥2 vs 1).
```

Reflection questions:  
- Which hypothesis (2 vs 3 clusters) receives strongest support?  
- Does the Bayesian analysis reproduce or challenge the Walktrap findings?  
...
BF_2v1   <- (p2/p1)   / (prior2/prior1)       # evidence: 2 vs 1 cluster
BF_3v2   <- (p3/p2)   / (prior3/prior2)       # evidence: 3 vs 2 clusters
BF_ge2v1 <- (p_ge2/p1) / (prior_ge2/prior1)   # evidence: ≥2 vs 1 cluster

BF_2v1; BF_3v2; BF_ge2v1
```

**Interpretation**  
- $BF_{2,1}$: Is there evidence for 2 clusters over 1?  
- $BF_{3,2}$: Does the data support 3 separate domains (authors’ hypothesis) vs. 2 domains (Walktrap result)?  
- $BF_{\geq 2,1}$: General evidence for clustering vs. no clustering at all.  
...
#3 Bonus: Using LLMs for Prior Specification
...
**In Part V, we have:**  
- Simulated networks under different priors (Bernoulli, Beta–Bernoulli, SBM).  
- Identified the structural assumptions each prior encodes.  
- Learned how to compute Bayes factors for edge inclusion and for clustering.  
- Applied the SBM workflow to real data on boredom, curiosity, and exploration.  
- Reflected on how priors translate substantive theory into statistical models.  

**Optional extensions (if time allows):**  
- Visualizing node-level clustering and co-clustering matrices.  
- Considering whether unstable assignments might suggest bridge symptoms.  

**Looking ahead:**  
- New work by **Sekulovski, Waaijers, & Arena (2025)**  
  ([preprint](https://osf.io/preprints/psyarxiv/k2twq_v1)) uses **large language models (LLMs)** to specify **hyperpriors** for network analysis.  
- This is implemented in the R package [bgmElicit](https://github.com/sekulovskin/bgmElicit) and in a Shiny app: [bgmElicit App](https://uvasobe.shinyapps.io/bgmElicit/).  
- ⚠️ Note: Using the R package currently requires an **OpenAI API key**.  
- These tools show how prior specification is an **active area of research**: moving from Bernoulli, to Beta–Bernoulli, to SBM, and now to AI-assisted elicitation.