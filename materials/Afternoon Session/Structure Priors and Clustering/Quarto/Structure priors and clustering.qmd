...
- In the morning session, we covered the fundamentals of Bayesian graphical modeling and learned how Bayesian reasoning distinguishes *evidence of absence* from *absence of evidence* using the inclusion Bayes factor.

- In the first part of the afternoon session we learned about testing for group differences in networks using the inclusion Bayes factor for edge differences.

- In this last part of the afternoon session, we will explore different **priors on the network structure** and how they can be used to encode different assumptions about the network topology. The focus will be on estimating single group networks. However, the Bernoulli and Beta-Bernoulli priors can also be used in the context of group comparisons.
...
-   Sekulovski, N., Waaijers, M., & Arena, G. (in press). LLM-based prior elicitation for Bayesian graphical modeling. British Journal of Mathematical and Statistical Psychology https://doi.org/10.1111/bmsp.70045
...
## 2.3 Correcting for Multiple Testing with the Beta–Bernoulli Prior


The Beta-Bernoulli prior corrects for multiple testing. This is also known as **multiplicity correction**.  
...
By contrast, the Beta–Bernoulli prior automatically adjusts the odds for edge inclusion, so that the evidence required for edge inclusion scales with the number of possible edges in the network. This is because the probability is spread out over a larger mass of network densities.
...
-   Some symptoms may connect domains, suggesting potential **bridges**, which some authors have argued are important in the study of comorbidity of mental disorders (e.g., [Cramer et al., 2010](https://www.cambridge.org/core/journals/behavioral-and-brain-sciences/article/abs/comorbidity-a-network-perspective/4A9FE571B49A9150DFABCD5845EC0B3D); [Borsboom & Cramer, 2013](https://www.annualreviews.org/content/journals/10.1146/annurev-clinpsy-050212-185608)).
...
The **Stochastic Block Model (SBM)** is a model from the random graph literature that allows us to model clustering in networks. By stipulating the SBM as a prior on the network structure of the OMRF, we say that the probability of an edge between two nodes depends on the clusters (blocks) to which the nodes belong. In other words: 
...
Beyond incorporating the theoretical assumption of clustering into the statistical model, a key practical goal of network analysis using the SBM prior is to test for the presence of clustering and to estimate the cluster memberships of the nodes.
...
By default the summarized output gives us the estimated cluster membership of the nodes based on the posterior mean and posterior mode. As well as the Bayes factor that tests the hypothesis of clustering  $\mathcal{H_1}: B > 1$ against no clustering $\mathcal{H_0}: B = 1$.

### Estimating the model and testing for clustering
...
summary(fit_sbm)
plot_network(fit_sbm)
```


Testing point hypotheses, such as, $\mathcal{H_1}: B = 2$ against $\mathcal{H_2}: B = 1$ (i.e., no clustering), can be done with the `clusterBayesfactor()` function:
...
- Inspect `posterior_mean_coclustering_matrix` to see how often pairs of nodes appear in the same cluster.  
...
#round(fit_sbm$sbm$posterior_mean_coclustering_matrix, 2)
```
...
A recent proposal by **Sekulovski, Waaijers, & Arena (2025)** ([link](https://bpspsychub.onlinelibrary.wiley.com/doi/10.1111/bmsp.70045)) introduces **LLM-based prior elicitation**.
