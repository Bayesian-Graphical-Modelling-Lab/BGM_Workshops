...
- **Estimate** networks within a single group  
  - Using Bayesian methods implemented in `bgms`, `BGGM`, and `BDgraph`.  
...
- **Evaluate** prior assumptions about network topology  
  ...
  - Understanding how priors affect posterior edge probabilities, density, and clustering.  

- **Interpret** Bayesian results in terms of  
  - Evidence for and against individual edges,  
  - Precision of parameter estimates,  
  - Influence of prior choices on conclusions.

---

## Goals for this session

We now extend these ideas to the case of **two (or more) groups**. At the end of this session, you should be able to:
...
- **Analyze** how the prior on the focal effect influences the Bayes factor, and evaluate the implications for interpreting group differences.  
...
- From estimating a single network → to comparing networks across groups.  
- From studying priors on topology → to priors on group differences.
...
In Session I, we saw the broader ecosystem of Bayesian network software.  
For this lecture the focus is narrower: **methods for comparing networks across groups**.  

For `bgms`, this is a very recent development. At present, only two packages implement principled Bayesian group comparison: `BGGM` and `bgms`. These capabilities have not yet been integrated into `easybgm` or `JASP` (although work is underway and the first functionality is expected later this year).  
...
|-------------------|------------------------|---------------------------------|---------------------------------|-------------------------|----------------------------------------------|
| **BGGM**</br><img src="figures/bggm_sticker.png" width="75px"> | Continuous (Gaussian)          | Edge estimation | **Single-model** + Ordinal constraints | ❌ No                     | Tests specific edge differences for Gaussian models |
| **bgms** </br><img src="figures/bgms_sticker.svg" width="75px"> | Binary, Ordinal                | Edge estimation + structure learning | **Single-Model** + **Model-averaged**   | ✅ Yes                    | Handles multiplicity; tests differences ordinal model parameters |  
...
- This is where **`easybgm`** usually comes in: it wraps around `bgms` to offer Bayes factor tables, and plotting. These conveniences have not yet been extended to group comparisons, but development is ongoing.
...
> Does this support the hypothesis of invariance? Why?
...
$$
\delta_{ij} = \theta^{(2)}_{ij} - \theta^{(1)}_{ij},
$$

where $\theta^{(g)}_{ij}$ is the edge parameter in group $g$.  
- If $\delta_{ij}=0$, the edge is **invariant** across groups.  
- If $\delta_{ij}\neq 0$, there is a **difference**.

In **Session I**, we asked if there is an edge, and used the Bayes factor to test if the partial assocation $\theta_{ij}$ is zero or not. Here, we ask if there is an edge difference, and use the Bayes factor to test if the difference effect $\delta_{ij}$ is zero or not.  
...
- Compared the two approaches in practice and reflected on their use for reporting and teaching.  
- Considered the role of **omnibus tests** and why they should be interpreted with caution.  

**Take-away:**  
Bayesian group comparisons provide a principled framework for studying invariance and differences in networks.  
They extend what we learned about edge evidence and prior specification to the **group level**, maintaining coherence with the Bayesian workflow and built-in control of multiplicity.
