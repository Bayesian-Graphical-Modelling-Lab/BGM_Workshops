...
- In the first part of the afternoon session we learned about testing for group differences in networks using the **Bayes factor for edge differences**.
...
As we saw during the morning session, the priors on the network structure determine the **prior odds** used in the calculation of the inclusion Bayes factor.

For the Bernoulli prior, the prior odds for each edge is simply:
$$
\frac{P(\mathcal{H}_1)}{P(\mathcal{H}_0)} = \frac{\pi}{1 - \pi}
$$
For the prior odds of the Beta–Bernoulli prior, we have two options for the prior odds. The first option defines the priors odds independently for each edge and is defined as

$$
\frac{P(\mathcal{H}_1)}{P(\mathcal{H}_0)} = \frac{\alpha}{\beta}
$$
This is the option that is implemented in the `easybgm` package.

However, another representation of the prior odds under the Beta–Bernoulli prior accounts for the current number of edges $k$ in the network and the total number of possible edges $m$.
