...
Let's visualize the implications of this prior for a small network with $p = 6$ nodes (i.e., $k = 15$ possible edges and $2^k$ possible network structures).

-   Left panel: prior probability assigned to **individual network structures**.
-   Right panel: prior probability distribution over **network density**.
...
- Expected density is fixed at $\pi$, regardless of network size.
But how should one choose $\pi$?\
...
Alternatively, based on substantive or theoretical considerations, we might believe that some edges are more likely than others. The Bernoulli structure prior allows us to incorporate such assumptions by setting a separate inclusion probability π for each edge.


### Prior odds for the inclusion Bayes factor 

As we saw in the morning lecture, the prior odds for edge inclusion under this prior are simply:

$$
\text{Prior Odds} = \frac{P(\mathcal{H}_1)}{P(\mathcal{H}_0)} = \frac{\pi}{1 - \pi}
$$
...
For GGMs and mixed data, the Bernoulli prior is the only choice and is automatically set.
By default, the inclusion probability π is set to $0.5$ for all edges, using the `inclusion_prob` argument.
Here is an example:
...
Under the Beta–Bernoulli prior, the prior odds for edge inclusion are given by (when they do not depend on $k$) are:

$$
\text{Prior Odds} = \frac{P(\mathcal{H}_1)}{P(\mathcal{H}_0)} = \frac{\alpha}{\beta}
$$

## 2.2 Workflow in easybgm

This prior is only available both for networks based on ordinal/binary data (i.e., using the ordinal Markov random field).
...
As a guide on deciding how to set the $\alpha$ and $\beta$ parameters, you can use [this](https://amshenao.shinyapps.io/FINAL/) Shiny app, which can help you to visualize the distribution. The app also provides the measures of central tendency and dispersion of the distribution, given the specified parameters.
...
We need a prior that directly encodes the clustering structure.
...
2.  **Edge probabilities**
    -   For each pair of nodes, an edge probability $\pi_{aa}$ is drawn from a Beta($\alpha_{within}, \beta_{within}$), for edges belonging to the same cluster and and edge probability $\pi_{ab}$ is drawn from a Beta($\alpha_{between}, \beta_{between}$), for edges belonging to different clusters.
3.  **Edges**
...
The central goal of SBM analysis is to evaluate evidence for clustering and to estimate the cluster membership of the nodes. 
...
In this example, we will be using a simulated data set with 12 variables, where we know that the true number of clusters is 2, in order to illustrate the workflow and how the method can capture the clustering structure. We will leave all the default hyperparameter settings, except for λ, which we will set to 1. This setting reflects our prior belief that there is likely to be one cluster, and we will leave the hyperparameters for the Beta distribution to 1. 
...
```{r}
# Clustering Bayes Factor for point hypotheses
library(easybgm)
clusterBayesfactor(fit_sbm, b1= 1, b2 = 2)
```
...
-   Discussed how each prior encodes different assumptions about edge inclusion, network density, and 
   clustering.\
